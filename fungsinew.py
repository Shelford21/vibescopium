...
def check_reviews_threshold(num_reviews):
    if num_reviews == 0:
        ...
        return f"⚠️ Only {num_reviews} reviews found. Analysis may be less accurate. To load the dataframes, please access to Dataframes on the sidebar"
        ...
        return f"🎯 {num_reviews} reviews found. Perform sentiment analysis with enough data! . To load the dataframes, please access to Dataframes on the sidebar"
...
⚠️ Reviews that are below 5000 are considered low on data , therefore the prediction may be less accurate</h4>
    ...
    if st.session_state['app_id'] :
        ...
        if st.button("Fetch reviews") :
            with st.spinner("Fetching reviews... (If there are many reviews, then scraping will take 1-2 minutes)"):
                reviews,_ = reviews(
                    app_id,
                    lang='id',
                    country='id',
...
if st.session_state["current_page"] == "😀 Positive":
    ...
    try:
        ...
        st.markdown(
            """
            <div class="transparent-container">
                <h5>On the other hand, if words like "bad" 👎, "worst" 😡, or "disappointed" 😞 are among the most common, it indicates negative sentiment. Users tend to use these words when expressing frustration, dissatisfaction, or regret about an experience, product, or event.<br><br>For instance, if "slow" 🐢 is frequently mentioned, it might be related to complaints about service delays or performance issues. If "broken" 💔 appears often, it suggests that many users are reporting defects or malfunctions.<br><br>In cases where words like "scam" 🚨 or "waste" 🗑️ are dominant, the dataset could contain reviews about fraudulent activities or poor-quality experiences. These words provide insights into the pain points and common complaints users face.<br><br>By analyzing both positive and negative word patterns, we gain a deeper understanding of how users perceive a given topic. The visualization allows us to identify key themes, track customer satisfaction, and even detect potential issues that need addressing.</h5>
            </div>
    ...
    try:
            ...
            def preprocess_tfidf(df):
                if "tfidf_vectorizer" not in st.session_state:
                    ...
                    X_tfidf = st.session_state.tfidf_vectorizer.fit_transform(clean_df['text_akhir'])
                else:
                    X_tfidf = st.session_state.tfidf_vectorizer.transform(clean_df['text_akhir'])
